...
public class MyPIDController {
     ...
     */
    public MyPIDController(double kp, double ki, double kd, double kf, double sp, double pv) {
        kP = kp;
        kI = ki;
        kD = kd;
        kF = kf;
    ...
    public double calculate(double pv) {
        ...
        // returns u(t)
        double power = kP * errorVal_p + kI * totalError + kD * errorVal_v + kF * setPoint;
        if (currentPower == 0) {
            currentPower = power;
            return power;
        }
        else {
            currentPower = currentPower +currentPower * power * scaleFactor;
            return currentPower +currentPower * power * scaleFactor;
        }
    }
